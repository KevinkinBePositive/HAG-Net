...
#由于小波卷积无法改变通道数，故编写一个WTConv2d和一个1*1卷积
class WTConv2d_tf(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=1, stride=1, bias=True):
        super(WTConv2d_tf, self).__init__()

        # 定义1x1卷积，用于改变通道数
        self.conv1x1 = nn.Conv2d(in_channels, out_channels, kernel_size=kernel_size, stride=stride, padding=0, bias=bias)
